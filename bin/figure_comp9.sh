...
maxsize=$(cat <(pdfinfo figure1.pdf) <(pdfinfo figure2.pdf) <(pdfinfo figure3.pdf) <(pdfinfo figure4.pdf) <(pdfinfo figure5.pdf) <(pdfinfo figure6.pdf) <(pdfinfo figure7.pdf) <(pdfinfo figure8.pdf) <(pdfinfo figure9.pdf) | grep 'Page size:' | awk '{x=x<$3?$3:x; y=y<$5?$5:y}END{print x,y}')
...
pdfjam cfigure[1-9].pdf --delta '10px 20px' --papersize '{842px,5000px}' --nup 3x3 --outfile final.pdf 2>/dev/null
...
cropvals=$(cat <(pdfinfo cfinal.pdf) <(pdfinfo cd.pdf) | grep 'Page size:' | awk '{getline line; split(line,l," "); printf "%i %i %i %i", 0, $5/3, $3-l[3], $5-$5/3-l[5] }'); pdfcrop --margins "$cropvals" d.pdf cd.pdf 1>/dev/null
...
cropvals=$(cat <(pdfinfo cfinal.pdf) <(pdfinfo ce.pdf) | grep 'Page size:' | awk '{getline line; split(line,l," "); printf "%i %i %i %i", ($3+10)/3, $5/3+4, $3-($3+10)/3-l[3], $5-$5/3-l[5]-4 }'); pdfcrop --margins "$cropvals" e.pdf ce.pdf 1>/dev/null
...
cropvals=$(cat <(pdfinfo cfinal.pdf) <(pdfinfo cf.pdf) | grep 'Page size:' | awk '{getline line; split(line,l," "); printf "%i %i %i %i", ($3+10)*2/3, $5/3, $3-($3+10)*2/3-l[3], $5-$5/3-l[5] }'); pdfcrop --margins "$cropvals" f.pdf cf.pdf 1>/dev/null
...
cropvals=$(cat <(pdfinfo cfinal.pdf) <(pdfinfo cg.pdf) | grep 'Page size:' | awk '{getline line; split(line,l," "); printf "%i %i %i %i", 0, $5*2/3+4, $3-l[3], $5-$5*2/3-l[5]-4 }'); pdfcrop --margins "$cropvals" g.pdf cg.pdf 1>/dev/null
...
cropvals=$(cat <(pdfinfo cfinal.pdf) <(pdfinfo ch.pdf) | grep 'Page size:' | awk '{getline line; split(line,l," "); printf "%i %i %i %i", ($3+10)/3, $5*2/3, $3-($3+10)/3-l[3], $5-$5*2/3-l[5] }'); pdfcrop --margins "$cropvals" h.pdf ch.pdf 1>/dev/null
...
cropvals=$(cat <(pdfinfo cfinal.pdf) <(pdfinfo ci.pdf) | grep 'Page size:' | awk '{getline line; split(line,l," "); printf "%i %i %i %i", ($3+10)*2/3, $5*2/3, $3-($3+10)*2/3-l[3], $5-$5*2/3-l[5] }'); pdfcrop --margins "$cropvals" i.pdf ci.pdf 1>/dev/null
