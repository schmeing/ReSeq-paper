...
def get_distinct_colors(nr):
    ...
    hexcols = ["#D92120","#488BC2","#7FB972","#E6642C","#781C81","#D9AD3C","#BBBBBB","#4065B1"]
    return hexcols[:nr]
    ...
    for d in data:
        if( BRANCH_CLASSIFICATION_NAME in d ):
            ...
            ax.plot(kmers, branch_rate, '-',
                    marker=d['plot_marker'], color=d['plot_color'], lw=LINE_WIDTH)
            names.append(d['name'])
    ...
    for d in data:
        if( QUALITY_SCORE_NAME in d and
            ...
            ax.plot(indices, mean_quality, '-',
                    marker=marker, color=d['plot_color'], lw=LINE_WIDTH)
    #pl.ylim([0, 40])
...
def load_preqc_datafiles(preqc_files):
    ...
    plot_colors = get_distinct_colors(len(preqc_files)) # returns up to 12
    # load the data
    data = []
    deserial_fail = []
    for f in preqc_files:
        ...
        try:
            ...
            plot_colors.pop(len(data)) # Remove the colors that are assigned to failed files
            continue
